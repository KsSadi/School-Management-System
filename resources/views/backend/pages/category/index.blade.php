...
@section('subcontent')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            @if( auth('admin')->user() && ( auth('admin')->user()->can('category.create')))
                <button class="btn btn-primary shadow-md mr-2" type="button" id="addNew">Add New Category</button>
            @endif
        </div>
    ...
    <script>
        ...
        $(document).ready(function () {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (data.status == 'success') {
                                fetchListData("{{route('dashboard.category.list')}}");
                                Swal.fire(
                                    'Deleted!',
                                    data.message,
                                    'success'
            ...
            $("#addNew").on('click', () => {
                let url = "{{route('dashboard.category.create')}}";
                ajaxGet(url, 'GET', {}, showData);
            });
