...
@section('subcontent')
    ...
    <h2 class="intro-y text-lg font-medium mt-10">Vendor  List </h2>
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            @if( auth('admin')->user() && ( auth('admin')->user()->can('vendor.create')))
                <button class="btn btn-primary shadow-md mr-2" type="button" id="addNew">Add New Vendor</button>
            @endif
        </div>
        <div id="listContainer" class="intro-y col-span-12 overflow-auto lg:overflow-visible">
            @include('backend.pages.vendor.list')
        </div>
    ...
    <script>
        ...
        $(document).ready(function () {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (data.status == 'success') {
                                fetchListData("{{route('dashboard.vendor.list')}}");
                                Swal.fire(
                                    'Deleted!',
                                    data.message,
                                    'success'
            ...
            $("#addNew").on('click', () => {
                let url = "{{route('dashboard.vendor.create')}}";
                ajaxGet(url, 'GET', {}, showData);
            });
