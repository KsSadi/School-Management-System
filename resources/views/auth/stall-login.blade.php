...
@section('head')
    <title>Login - BookStall Owner</title>
    <style>
        ...
        <div class="block xl:grid grid-cols-2 gap-4">
            ...
            <div class="h-screen xl:h-auto flex py-5 xl:py-0 my-10 xl:my-0">
                <div class="my-auto mx-auto xl:ml-20 bg-white dark:bg-darkmode-600 xl:bg-transparent px-5 sm:px-8 py-8 xl:p-0 rounded-md shadow-md xl:shadow-none w-full sm:w-3/4 lg:w-2/4 xl:w-auto">
                    <h2 class="intro-x font-bold text-2xl xl:text-3xl text-center xl:text-left">Sign In as a Book Stall Owner</h2>
    ...
    <script>
        (function () {
            async function login() {
                ...
                axios.post(`{{route('stall.loginPost')}}`, {
                    email: email,
                    password: password
                }).then(res => {
                    console.log(res)
                    if (res.data.status === 'success') {
                        window.location.href = `{{route('stall.dashboard')}}`
                    } else {
